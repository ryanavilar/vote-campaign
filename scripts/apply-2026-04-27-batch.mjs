/**
 * 2026-04-27 batch:
 *   Form DPT (isi_form_dpt=Sudah, create member if missing): TN16,22,24,25,27,28,30,31,33
 *   Web DPT (status_dpt + registrasi_website_dpt = Sudah, create with all 3 if missing): TN16,24,25,27,28,29,30,31
 */
...
const formAng = [16, 22, 24, 25, 27, 28, 30, 31, 33];
const webAng = [16, 24, 25, 27, 28, 29, 30, 31];
...
async function processForm(ang) {
  ...
  const { data: members } = await supabase.from("members").select("id, alumni_id, nama, isi_form_dpt").in("alumni_id", ids);
