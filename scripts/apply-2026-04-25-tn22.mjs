/**
 * 2026-04-25: TN22 form DPT ingestion
 *   (TN22 web DPT handled by apply-web-dpt-by-list.mjs 22 scripts/_tn22-web-dpt.txt)
 */
...
const ANG = 22;
const records = JSON.parse(readFileSync("/tmp/tn22-form-records.json", "utf8"));
console.log(`Mode: ${APPLY ? "APPLY" : "DRY-RUN"} | TN${ANG} records: ${records.length}\n`);
...
const { data: members } = await supabase
  .from("members").select("id, alumni_id, nama, isi_form_dpt, no_hp")
  .in("alumni_id", uniq.map((m) => m.alumni.id));
...
console.log(`flip=${toFlip.length} already=${alreadySudah.length} create=${needCreate.length}`);
...
if (needCreate.length) {
  ...
  const rows = needCreate.map((m) => ({
    no: nextNo++, nama: m.alumni.nama, angkatan: m.alumni.angkatan,
